			...
			for _, version := range fivs.Versions {
				...
				for range 3 {
					...
					if err != nil && !ignore {
						...
						if bi.Name == kypelloMetaBucket && strings.Contains(version.Name, dataUsageCacheName) {
							ignore = true
							stopFn(version.Size, err)
							decomLogIf(ctx, err)
							break
	...
	for _, set := range pool.sets {
		for _, bi := range buckets {
			...
			if err = set.listObjectsToDecommission(ctx, bi, func(entry metaCacheEntry) {
				...
				if bi.Name == kypelloMetaBucket && strings.Contains(entry.name, dataUsageCacheName) {
...
func (z *erasureServerPools) getBucketsToDecommission(ctx context.Context) ([]decomBucketInfo, error) {
	...
	decomMetaBuckets := []decomBucketInfo{
		{
			Name:   kypelloMetaBucket,
			Prefix: kypelloConfigPrefix,
		},
		{
			Name:   kypelloMetaBucket,
...
func (z *erasureServerPools) StartDecommission(ctx context.Context, indices ...int) (err error) {
	...
	// Create .kypello.sys/config, .kypello.sys/buckets paths if missing,
	// this code is present to avoid any missing meta buckets on other
	// pools.
	for _, metaBucket := range []string{
		pathJoin(kypelloMetaBucket, kypelloConfigPrefix),
		pathJoin(kypelloMetaBucket, bucketMetaPrefix),
	} {
