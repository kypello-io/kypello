...
TOOL_DIR = .tool
BINNY = $(shell command -v binny 2>/dev/null || echo go run github.com/anchore/binny/cmd/binny@latest)
GOLANGCI = $(TOOL_DIR)/golangci-lint
TYPOS = $(TOOL_DIR)/typos
...
getdeps: ## fetch necessary dependencies
	@mkdir -p ${GOPATH}/bin
	@echo "Installing tool dependencies via binny"
	@$(BINNY) install
...
lint: getdeps ## runs golangci-lint suite of linters
	...
	@$(TYPOS) ./ || echo "typos binary is not found.. skipping.."
...
build: checks build-debugging ## builds minio to $(PWD)
	@echo "Building kypello binary to './kypello'"
	@CGO_ENABLED=0 GOOS=$(GOOS) GOARCH=$(GOARCH) go build -tags kqueue -trimpath --ldflags "$(LDFLAGS)" -o $(PWD)/kypello 1>/dev/null
...
docker-hotfix: hotfix-push checks ## builds minio docker container with hotfix tags
	@echo "Building kypello docker image '$(TAG)'"
...
install-race: checks build-debugging ## builds kypello to $(PWD)
	@echo "Building kypello binary with -race to './kypello'"
	@GORACE=history_size=7 CGO_ENABLED=1 go build -tags kqueue,dev -race -trimpath --ldflags "$(LDFLAGS)" -o $(PWD)/kypello 1>/dev/null
	@echo "Installing kypello binary with -race to '$(GOPATH)/bin/kypello'"
	@mkdir -p $(GOPATH)/bin && cp -af $(PWD)/kypello $(GOPATH)/bin/kypello

install: build ## builds kypello and installs it to $GOPATH/bin.
	@echo "Installing kypello binary to '$(GOPATH)/bin/kypello'"
	@mkdir -p $(GOPATH)/bin && cp -af $(PWD)/kypello $(GOPATH)/bin/kypello
	@echo "Installation successful. To learn more, try \"kypello --help\"."
...
clean: ## cleanup all generated assets
	...
	@rm -rvf .tool
	@rm -rvf .verify*
	@rm -rvf kypello-release
	@rm -rvf kypello.RELEASE*.hotfix.*
	@rm -rvf pkger_*.deb
