...
To host multiple tenants on a single machine, run one Kypello Server per tenant with a dedicated HTTPS port, configuration, and data directory.
...
```sh
kypello server --address :9001 /data/tenant1
kypello server --address :9002 /data/tenant2
kypello server --address :9003 /data/tenant3
```

![Example-1](https://github.com/kypello-io/kypello/blob/master/docs/screenshots/Example-1.jpg?raw=true)
...
```sh
kypello server --address :9001 /disk{1...4}/data/tenant1
kypello server --address :9002 /disk{1...4}/data/tenant2
kypello server --address :9003 /disk{1...4}/data/tenant3
```

![Example-2](https://github.com/kypello-io/kypello/blob/master/docs/screenshots/Example-2.jpg?raw=true)

## 2. Distributed Deployment

To host multiple tenants in a distributed environment, run several distributed Kypello Server instances concurrently.
...
kypello server --address :9001 http://192.168.10.1{1...4}/data/tenant1

export MINIO_ROOT_USER=<TENANT2_ACCESS_KEY>
export MINIO_ROOT_PASSWORD=<TENANT2_SECRET_KEY>
kypello server --address :9002 http://192.168.10.1{1...4}/data/tenant2

export MINIO_ROOT_USER=<TENANT3_ACCESS_KEY>
export MINIO_ROOT_PASSWORD=<TENANT3_SECRET_KEY>
kypello server --address :9003 http://192.168.10.1{1...4}/data/tenant3
```

**Note:** Execute the commands on all 4 nodes.

![Example-3](https://github.com/kypello-io/kypello/blob/master/docs/screenshots/Example-3.jpg?raw=true)

**Note**: On distributed systems, root credentials are recommend to be defined by exporting the `MINIO_ROOT_USER` and  `MINIO_ROOT_PASSWORD` environment variables. If no value is set MinIO setup will assume `kypelloadmin/kypelloadmin` as default credentials. If a domain is required, it must be specified by defining and exporting the `MINIO_DOMAIN` environment variable.
