# Kypello FTP/SFTP Server

Kypello natively supports FTP/SFTP protocol, this allows any ftp/sftp client to upload and download files.
...
Kypello supports following FTP/SFTP based protocols to access and manage data.
...
Start Kypello in a distributed setup, with 'ftp/sftp' enabled.

```
Kypello server http://server{1...4}/disk{1...4}
   --ftp="address=:8021" --ftp="passive-port-range=30000-40000" \
   --sftp="address=:8022" --sftp="ssh-private-key=/home/Kypellouser/.ssh/id_rsa"
...
...
```

Following example shows connecting via ftp client using `Kypelloadmin` credentials, and list a bucket named `runner`:

```
ftp localhost -P 8021
Connected to localhost.
220 Welcome to Kypello FTP Server
Name (localhost:user): kypelloadmin
...
Following example shows connecting via sftp client using `kypelloadmin` credentials, and list a bucket named `runner`:

```
sftp -P 8022 kypelloadmin@localhost
kypelloadmin@localhost's password:
Connected to localhost.
sftp> ls runner/
chunkdocs  testdir
```
...
> NOTE: if Kypello distributed setup is already configured to run under TLS, FTP will automatically use the relevant
