...
Refer to sizing guide for more understanding on default values chosen depending on your erasure stripe size [here](https://github.com/kypello-io/kypello/blob/master/docs/distributed/SIZING.md). Parity settings can be changed using [storage classes](https://github.com/kypello-io/kypello/tree/master/docs/erasure/storage-class).
...
- Servers running distributed Kypello instances should be less than 15 minutes apart. You can enable [NTP](http://www.ntp.org/) service as a best practice to ensure same times across servers.
...
Example 1: Start distributed Kypello instance on n nodes with m drives each mounted at `/export1` to `/exportm` (pictured below), by running this command on all the n nodes:

![Distributed MinIO, n nodes with m drives each](https://github.com/kypello-io/kypello/blob/master/docs/screenshots/Architecture-diagram_distributed_nm.png?raw=true)
...
kypello server http://host{1...n}/export{1...m}
```
...
export MINIO_ROOT_USER=<ACCESS_KEY>
export MINIO_ROOT_PASSWORD=<SECRET_KEY>
kypello server http://host{1...n}/export{1...m} http://host{o...z}/export{1...m}
```

For example:

```
kypello server http://host{1...4}/export{1...16} http://host{5...12}/export{1...16}
```
